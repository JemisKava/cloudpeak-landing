...
const CoreFeature = ({ features, index }: { features: CoreFeatureCardProps, index: number }) => {
  const { img, alt, description, title, metric } = features
  return (
    <div className={cn('flex items-center md:flex-col md:gap-y-8 sm:flex-col sm:gap-y-8 gap-x-1', index % 2 !== 0 && 'flex-row-reverse')}>
      ...
      <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, ease: 'easeInOut' }} viewport={{ once: true, amount: 0.3 }} className='w-full'>
        <div className={cn('ml-16 space-y-4 sm:ml-0', index % 2 !== 0 && 'lg:ml-0 mr-16')}>
